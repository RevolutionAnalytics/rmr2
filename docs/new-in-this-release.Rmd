...
# Removed feature
Unfortunately we couldn't make the 1-install feature, which had been experimentally introduced with the previous release, stable enough and we decided to pull it. The idea was to install missing packages on the fly on any nodes that needed them just before the map or the reduce functions are invoked. We are not giving up on the general idea but we are going to go back to the drawing board and try different approaches.