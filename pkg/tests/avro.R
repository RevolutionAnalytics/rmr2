...
test_avro_rmr <-   function(df,test,write.args=list(),input.format.args=list(),map=function(k,v)rmr.str(v)) {
...
expect_equivalent_avro_rmr(
  read.avro(system.file("data/yield1k.avro",package="ravro")),write.args=list(unflatten=T))
...
### flattening works
## complex is not equivalent
# Error retrieving schema.
# Verify that the file exists and is a valid Avro: /tmp/RtmpPzKpBf/file1e4d2b5588fc.avro
## complex still not working with flatten=F and without flatten parameter

test_that("flattening works", {
  ## The complex data set is not yet supported by ravro:::write.avro
  complex <- read.avro(complex.file.loc,flatten=T)
  #expect_equal_avro_rmr(complex)
...
################################# Read Example Avro Files #####################################

context("Read Example Avro Files")

### yield reads unflattended correctly
...
test_that("yield reads unflattened correctly", {
  ...
  # rmr2:::from.dfs cannot combine un-flattened data values
  #expect_equal_avro_rmr(yield)
})


### yield reads flattened correctly
# equivalent, but not equal, because rmr2 inserts a "class" attribute with value "AsIs" for the nonStandardData

test_that("yield reads flattened correctly", {
  yield <- read.avro(file.path(system.file("data",package="ravro"),"yield1k.avro"),flatten=T)
  expect_equivalent_avro_rmr(yield,write.args=list(unflatten=T))
})


### as-planted reads unflattened correctly
...
test_that("as-planted reads unflattened correctly", {
  #as_planted <- read.avro(file.path(system.file("data",package="ravro"),"as-planted1k.avro"),
                          flatten=F)
  # rmr2:::from.dfs cannot combine un-flattened data values
  #expect_equivalent_avro_rmr(as_planted)
})


### as-planted reads flattened correctly
# equivalent, but not equal, because rmr2 inserts a "class" attribute with value "AsIs" for the nonStandardData

test_that("as-planted reads flattened correctly", {
  as_planted <- read.avro(file.path(system.file("data",package="ravro"),"as-planted1k.avro"),
                          flatten=T)
  expect_equivalent_avro_rmr(as_planted,write.args=list(unflatten=T))  
})
